...
- Release pipeline: on every version tag, GitHub Actions runs manifest validation and the full test suite, builds the release zip with the project packager, verifies the tag matches module.json, and attaches the zip and manifest to the GitHub release with notes extracted from the changelog.
